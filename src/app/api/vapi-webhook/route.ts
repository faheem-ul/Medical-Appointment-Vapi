...
const transport = nodemailer.createTransport({
  ...
  auth: {
    ...
    pass: "pyvg cksu ueom bnlr", // move to ENV in production
  },
  tls: {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const durationSeconds =
      message?.duration ?? message?.call?.duration ?? null;

    const finalCallDuration =
      callDuration || (durationSeconds ? `${durationSeconds} seconds` : "");
